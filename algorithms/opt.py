...
def optimal_iter(t, r_t, reqs_in, reqs_lp_t_in, X_t_in, Y_t_in, profit_in, file_name):
    profit = 0
    reject = [0,0,0]
    violat = 0
    migrat = 0
    ...
    if t == 0:
        u   = [[m.addVar(name=f"u_{idx_p}_{chi}", vtype=gp.GRB.BINARY) for chi in range(sys.CHI_MAX+1)] for idx_p in range(len_V_P_S)]
        u_h = [m.addVar(name=f"u_h_{idx_p}", vtype=gp.GRB.BINARY) for idx_p in range(len_V_P_S)]
    else:
        m.update()
        m = gp.read(str(file_name))
        m.update()
        print("Hhhehheheheheh")
    ...
    x.append([[m.addVar(name=f"x_{len_reqs-1}_{idx_v}_{idx_p}", vtype=gp.GRB.BINARY) for idx_p in range(len_V_P_S)] for idx_v in range(len_vservers)])
    y.append([[[[m.addVar(name=f"y_{len_reqs-1}_{idx_v}_{idx_q}_{f}_{k}", vtype=gp.GRB.BINARY) for k in range(K_MAX)] for f in range(F_MAX)] for idx_q in range(len_V_P_R)] for idx_v in range(len_vrus)])
    z.append([[[m.addVar(name=f"z_{len_reqs-1}_{idx_e}_{idx_l}_{s}", vtype=gp.GRB.BINARY) for s in range(S_MAX)] for idx_l in range(len_L)] for idx_e in range(len_vpaths)])
    ...
    # Auxiliary Variables
    a.append([[m.addVar(name=f"a_{len_reqs-1}_{idx_v}_{idx_p}", vtype=gp.GRB.BINARY) for idx_p in range(len_V_P_S)] for idx_v in range(len_vpaths)])
    a_h.append([[m.addVar(name=f"a_h_{len_reqs-1}_{idx_v}_{idx_p}", vtype=gp.GRB.BINARY) for idx_p in range(len_V_P_R)] for idx_v in range(len_vpaths)])
    o.append([[m.addVar(name=f"o_{len_reqs-1}_{idx_v}_{idx_e}", vtype=gp.GRB.BINARY) for idx_e in range(len_E_P)] for idx_v in range(len_vpaths)])
    o_h.append([m.addVar(name=f"o_h_{len_reqs-1}_{idx_v}", vtype=gp.GRB.CONTINUOUS) for idx_v in range(len_vpaths)])
    o_h1.append([m.addVar(name=f"o_h1_{len_reqs-1}_{idx_v}", vtype=gp.GRB.BINARY) for idx_v in range(len_vpaths)])
    o_h2.append([m.addVar(name=f"o_h2_{len_reqs-1}_{idx_p}", vtype=gp.GRB.BINARY) for idx_p in range(len_V_P_S)])
    o_h3.append([m.addVar(name=f"o_h3_{len_reqs-1}_{idx_q}", vtype=gp.GRB.BINARY) for idx_q in range(len_V_P_R)])
    o_h4.append([m.addVar(name=f"o_h4_{len_reqs-1}_{idx_l}", vtype=gp.GRB.BINARY) for idx_l in range(len_L)])
    ...
    if (status == gp.GRB.OPTIMAL) and (timeout == 0):
        ...
        g_dep_opt = sum(vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,idx_l,s)] * r.Theta[idx_e_p]
                     ...
                     for s            in range(len(l.B_MAX))
                            ...
                            for idx_v, v in enumerate(r.V_S)
                            for idx_p, p in enumerate(V_P_S)
                           ) + sum(k * vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)] * r.Phi[idx_q]
                                   for idx_r, r in enumerate(reqs)
                                   for idx_w, w in enumerate(r.V_R)
                                   for idx_q, q in enumerate(V_P_R)
                                   for f in range(len(q.Pi_MAX))
        ...
        g_K_opt = max((r.rho_K * w.K_REQ) - (r.rho_K * sum(k*vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)]
                                                           for idx_q, q in enumerate(V_P_R)
                                                           for f in range(len(q.Pi_MAX))
                                                           for k in range(w.K_REQ + 1)
                                                          ))
        ...
        h_B_1 = [[1 - sum(vars_opt["a_{}_{}_{}".format(idx_r,idx_e,idx_p)]
                          for idx_p, p in enumerate(V_P_S)
                         )
                  for idx_e, e in enumerate(r.E)
                 ]
        ...
        h_B_2 = [[1 - sum(vars_opt["a_h_{}_{}_{}".format(idx_r,idx_e,idx_q)]
                          for idx_q, q in enumerate(V_P_R)
                         )
                  for idx_e, e in enumerate(r.E)
                 ]
        ...
        g_sum_1_opt = sum(vars_opt["u_{}_{}".format(idx_p,chi)]
                          for idx_p, p in enumerate(V_P_S)
                          for chi      in range(sys.CHI_MAX)
                         )
        g_sum_2_opt = sum(vars_opt["u_h_{}".format(idx_p)]
                      for idx_p, p in enumerate(V_P_S)
                     )
        g_sum_3_opt = sum(vars_opt["a_{}_{}_{}".format(idx_r,idx_e,idx_p)]
                      for idx_r, r in enumerate(reqs)
                      for idx_e, e in enumerate(r.E)
                      if  (e.v in r.V_S) and (e.w in r.V_S)
                      for idx_p, p in enumerate(V_P_S)
                     )
        g_sum_4_opt = sum(vars_opt["a_h_{}_{}_{}".format(idx_r,idx_e,idx_q)]
                      for idx_q, q in enumerate(V_P_R)
                      for idx_r, r in enumerate(reqs)
                      for idx_e, e in enumerate(r.E)
                      if  (e.v in r.V_R) and (e.w in r.V_R)
                     )
        g_sum_5_opt = sum(vars_opt["o_{}_{}_{}".format(idx_r,idx_e,idx_e_p)]
                      for idx_r,   r   in enumerate(reqs)
                      for idx_e,   e   in enumerate(r.E)
                      for idx_e_p, e_p in enumerate(E_P)
                     )
        g_sum_6_opt = sum(vars_opt["o_h_{}_{}".format(idx_r,idx_e)]
                      for idx_r, r in enumerate(reqs)
                      for idx_e, e in enumerate(r.E)
                     )
        ...
        g_dep_c = sum(vars_opt["c_{}_{}_{}".format(idx_r,idx_v,idx_p)] * r.Omega[idx_p]
                            for idx_r, r in enumerate(reqs)
                            for idx_v, v in enumerate(r.V_S)
                            for idx_p, p in enumerate(V_P_S)
                           )
        g_dep_b = sum(vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,idx_l,s)] * r.Theta[idx_e_p]
                     for idx_r, r     in enumerate(reqs)
                     for idx_e, e     in enumerate(r.E)
                     for idx_e_p, e_p in enumerate(E_P)
                     for idx_l, l     in enumerate(L_pqi[idx_e_p])
        ...
        g_dep_y = sum(k * vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)] * r.Phi[idx_q]
                                   for idx_r, r in enumerate(reqs)
                                   for idx_w, w in enumerate(r.V_R)
                                   for idx_q, q in enumerate(V_P_R)
                                   for f in range(len(q.Pi_MAX))
        ...
        print("Revenue:",sum(r.R for r in reqs))
        print("Deploymenttttttttttt: c, b, y",g_dep_c, g_dep_b, g_dep_y)
...
#             print(f"{var_name}: {value}")
        ...
        X_t = [[[vars_opt["x_{}_{}_{}".format(idx_r,idx_v,idx_p)] for idx_p, p in enumerate(V_P_S)] for idx_v, v in enumerate(r.V_S)] for idx_r, r in enumerate(reqs)]
        Y_t = [[[[[vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)] for k in range(K_MAX)] for f in range(F_MAX)] for idx_q, q in enumerate(V_P_R)] for idx_w, w in enumerate(r.V_R)] for idx_r, r in enumerate(reqs)]
