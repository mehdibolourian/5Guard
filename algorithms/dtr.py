...
def dtr_iter(LIMIT, profit_prev, iter, t, r_t, R_t, V_P_S, V_P_R, E_P, E_P_l, L, L_pqi, f_fgr, X_t, Y_t):
    #reqs_lp_t = []
    reqs = R_t.copy()
    ...
    x = [[[m.getVarByName(f"x_{idx_r}_{idx_v}_{idx_p}")
           ...
           else m.addVar(name=f"x_{idx_r}_{idx_v}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
    ...
    y = [[[[[m.getVarByName(f"y_{idx_r}_{idx_v}_{idx_q}_{f}_{k}")
             ...
             else m.addVar(name=f"y_{idx_r}_{idx_v}_{idx_q}_{f}_{k}", vtype=gp.GRB.CONTINUOUS, ub=1)
    ...
    z = [[[[m.getVarByName(f"z_{idx_r}_{idx_e}_{idx_l}_{s}")
            ...
            else m.addVar(name=f"z_{idx_r}_{idx_e}_{idx_l}_{s}", vtype=gp.GRB.CONTINUOUS, ub=1)
    ...
    a_x1 = [[m.getVarByName(f"a_x1_{idx_r}_{idx_p}")
             ...
             else m.addVar(name=f"a_x1_{idx_r}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
             for idx_p in range(len_V_P_S)]
            for idx_r in range(len_R)]
    ...
    a_x4 = [[[m.getVarByName(f"a_x4_{idx_r}_{idx_v}_{idx_p}")
              ...
              else m.addVar(name=f"a_x4_{idx_r}_{idx_v}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
              for idx_p in range(len_V_P_S)]
    ...
    a_y1 = [[m.getVarByName(f"a_y1_{idx_r}_{idx_q}")
             ...
             else m.addVar(name=f"a_y1_{idx_r}_{idx_q}", vtype=gp.GRB.CONTINUOUS, ub=1)
             for idx_q in range(len_V_P_R)]
            for idx_r in range(len_R)]
    a_y2 = [[[m.getVarByName(f"a_y2_{idx_r}_{idx_v}_{idx_p}")
              if m.getVarByName(f"a_y2_{idx_r}_{idx_v}_{idx_p}") is not None
              else m.addVar(name=f"a_y2_{idx_r}_{idx_v}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
    ...
    a_z1 = [[m.getVarByName(f"a_z1_{idx_r}_{idx_l}")
             ...
             else m.addVar(name=f"a_z1_{idx_r}_{idx_l}", vtype=gp.GRB.CONTINUOUS, ub=1)
             for idx_l in range(len_L)]
            for idx_r in range(len_R)]
    a_z2 = [[[m.getVarByName(f"a_z2_{idx_r}_{idx_v}_{idx_e}")
              if m.getVarByName(f"a_z2_{idx_r}_{idx_v}_{idx_e}") is not None
              else m.addVar(name=f"a_z2_{idx_r}_{idx_v}_{idx_e}", vtype=gp.GRB.CONTINUOUS, ub=1)
    ...
    a_z4 = [[m.getVarByName(f"a_z4_{idx_r}_{idx_v}")
             ...
             else m.addVar(name=f"a_z4_{idx_r}_{idx_v}", vtype=gp.GRB.CONTINUOUS, ub=1)
             for idx_v in range(len_E)]
            for idx_r in range(len_R)]

    # ## Auxiliary variables
    if len(R_t) == 0: ## No model is defined yet --> No a_x2 or a_x3 defined
        a_x2 = [[m.addVar(name=f"a_x2_{idx_p}_{chi}", vtype=gp.GRB.CONTINUOUS, ub=1)
                 for chi in range(sys.CHI_MAX+1)]
                for idx_p in range(len_V_P_S)]
        a_x3 = [m.addVar(name=f"a_x3_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
                for idx_p in range(len_V_P_S)]
    ...
    ##  Main Variables
    #   Only append the variable for the incoming SR and reuse the other variables from the last instance of the problem
    x.append([[m.addVar(name=f"x_{len_R-1}_{idx_v}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
               for idx_p in range(len_V_P_S)]
              for idx_v in range(len_V_S)])
    y.append([[[[m.addVar(name=f"y_{len_R-1}_{idx_v}_{idx_q}_{f}_{k}", vtype=gp.GRB.CONTINUOUS, ub=1)
                 for k in range(K_MAX)]
                for f in range(F_MAX)]
               for idx_q in range(len_V_P_R)]
              for idx_v in range(len_V_R)])
    z.append([[[m.addVar(name=f"z_{len_R-1}_{idx_e}_{idx_l}_{s}", vtype=gp.GRB.CONTINUOUS, ub=1)
                for s in range(S_MAX)]
               for idx_l in range(len_L)]
              for idx_e in range(len_E)])
    ...
    # Auxiliary Variables
    a_x1.append([m.addVar(name=f"a_x1_{len_R-1}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
                 for idx_p in range(len_V_P_S)])
    a_x4.append([[m.addVar(name=f"a_x4_{len_R-1}_{idx_e}_{idx_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
                  for idx_p in range(len_V_P_S)]
                 for idx_e in range(len_E)])

    a_y1.append([m.addVar(name=f"a_y1_{len_R-1}_{idx_q}", vtype=gp.GRB.CONTINUOUS, ub=1)
                 for idx_q in range(len_V_P_R)])
    a_y2.append([[m.addVar(name=f"a_y2_{len_R-1}_{idx_e}_{idx_q}", vtype=gp.GRB.CONTINUOUS, ub=1)
                  for idx_q in range(len_V_P_R)]
                 for idx_e in range(len_E)])

    a_z1.append([m.addVar(name=f"a_z1_{len_R-1}_{idx_l}", vtype=gp.GRB.CONTINUOUS, ub=1)
                 for idx_l in range(len_L)])
    a_z2.append([[m.addVar(name=f"a_z2_{len_R-1}_{idx_e}_{idx_e_p}", vtype=gp.GRB.CONTINUOUS, ub=1)
                  for idx_e_p in range(len_E_P)]
                 for idx_e in range(len_E)])
    ...
    if (status == gp.GRB.OPTIMAL) and (timeout == 0):
        ...
        x_opt_lp = [[[vars_opt["x_{}_{}_{}".format(idx_r,idx_v,idx_p)]
                     ...
                     for idx_v, v in enumerate(r.V_S)]
                    for idx_r, r in enumerate(reqs)]
        x_opt    = [[[0 for idx_p, p in enumerate(V_P_S)]
                     for idx_v, v in enumerate(r.V_S)]
                    for idx_r, r in enumerate(reqs)]

        c_opt_lp = [[[vars_opt["c_{}_{}_{}".format(idx_r,idx_v,idx_p)]
                      for idx_p, p in enumerate(V_P_S)]
                     for idx_v, v in enumerate(r.V_S)]
                    for idx_r, r in enumerate(reqs)]
        ...
        z_opt_lp = [[[[vars_opt["z_{}_{}_{}_{}".format(idx_r,idx_e,idx_l,s)]
                     ...
                     for idx_e, e in enumerate(r.E)]
                    for idx_r, r in enumerate(reqs)]
        z_opt    = [[[[0 for s in range(S_MAX)]
                      for l in L]
                     for idx_e, e in enumerate(r.E)]
                    for idx_r, r in enumerate(reqs)]

        b_opt_lp = [[[[[vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,idx_l,s)]
                        for s in range(S_MAX)]
                       for idx_l,l in enumerate(L_pqi[idx_e_p])]
                      for idx_e_p, e_p in enumerate(E_P)]
                     for idx_e, e in enumerate(r.E)]
                    for idx_r,r in enumerate(reqs)]
        b_opt    = [[[[[0 for s in range(S_MAX)]
                       for idx_l, l in enumerate(L_pqi[idx_e_p])]
                      for idx_e_p, e_p in enumerate(E_P)]
                     for idx_e, e in enumerate(r.E)]
                    for idx_r,r in enumerate(reqs)]
        ...
        constr[1] = all(sum(vars_opt["a_y1_{}_{}".format(idx_r,idx_q)]
                            for idx_r, r in enumerate(reqs)
                            if r.gamma == 2) <= 1
                        for idx_q, q in enumerate(V_P_R)
                       )

        constr[2] = all(vars_opt["x_{}_{}_{}".format(idx_r,idx_v,idx_p)]
                        <= 1 - sum(vars_opt["a_x1_{}_{}".format(idx_r1,idx_p)]
                                   for idx_r1, r1 in enumerate(reqs)
                                   if r1.gamma == 2 and idx_r1 != idx_r
                                  )
                        for idx_r, r in enumerate(reqs)
        ...
        constr[3] = all(sum(vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)]
                                for f in range(len(q.Pi_MAX))
                                for k in range(1, K_REQ_EFF[idx_r][idx_w] + 1)
                            ) <= 1 - sum(vars_opt["a_y1_{}_{}".format(idx_r1,idx_q)]
                                         for idx_r1, r1 in enumerate(reqs)
                                         if r1.gamma == 2 and idx_r1 != idx_r
                                        )
                        for idx_r, r in enumerate(reqs)
        ...
        constr[4] = all(sum(vars_opt["c_{}_{}_{}".format(idx_r,idx_v,idx_p)]
                            for idx_r, r in enumerate(reqs)
                            for idx_v, v in enumerate(r.V_S)
                           ) 
        ...
        constr[6] = all(sum(k * vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)]
                            for idx_r, r in enumerate(reqs)
                            for idx_w, w in enumerate(r.V_R)
                            for k in range(1, K_REQ_EFF[idx_r][idx_w] + 1)
                           ) * sys.Pi_PRB
                        + sum(vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)]
                                      for idx_r, r in enumerate(reqs)
                                      if  r.gamma == 1
                                      for idx_w, w in enumerate(r.V_R)
                                      for k in range(1, K_REQ_EFF[idx_r][idx_w] + 1)
        ...
        constr[7] = all(sum(vars_opt["a_z1_{}_{}".format(idx_r,idx_l)]
                            for idx_r, r in enumerate(reqs)
                            if r.gamma == 2
                           ) <= 1
                        for idx_l, l in enumerate(L)
        ...
        constr[8] = all(sum(vars_opt["z_{}_{}_{}_{}".format(idx_r,idx_e,idx_l,s)]
                            for s in range(S_MAX)
                        )
                        <= 1 - sum(vars_opt["a_z1_{}_{}".format(idx_r1,idx_l)]
                                   for idx_r1, r1 in enumerate(reqs)
                                   if (r1.gamma == 2) and (idx_r1 != reqs.index(r))
                                  )
                        for idx_r, r in enumerate(reqs)
                        for idx_e, e in enumerate(r.E)
                        for idx_l, l in enumerate(L)
        ...
        constr[9] = all(sum(vars_opt["z_{}_{}_{}_{}".format(idx_r,idx_e,idx_l,s)] * sys.B_WGB
                            for idx_r, r in enumerate(reqs)
                            if r.gamma == 1
                            for idx_e, e in enumerate(r.E)
                            for idx_e_p, e_p in enumerate(E_P)
                           ...
                           + sum(vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,L_pqi[idx_e_p].index(l),s)]
                                 for idx_r, r in enumerate(reqs)
                                 for idx_e, e in enumerate(r.E)
                                 for idx_e_p, e_p in enumerate(E_P)
                                 if e_p in E_P_l[idx_l]
