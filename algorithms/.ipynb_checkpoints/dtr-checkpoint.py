...
def lp_round_iter(t, r_t, reqs_in, reqs_lp_t_in, X_t_in, Y_t_in, profit_in, file_name):
    profit = 0
    reject = [0,0,0]
    violat = 0
    migrat = 0
    ...
    if (status == gp.GRB.OPTIMAL) and (timeout == 0):
        ...
        for idx_r,r in enumerate(reqs):
            for idx_e, e in enumerate(r.E):
                for idx_p, p in enumerate(V_P_S):
                    if a_opt_lp[idx_r][idx_e][idx_p] > LIMIT:
                        a_opt[idx_r][idx_e][idx_p] = 1
        ...
        for idx_r,r in enumerate(reqs):
            for idx_e, e in enumerate(r.E):
                for idx_q, q in enumerate(V_P_R):
                    if a_h_opt_lp[idx_r][idx_e][idx_q] > LIMIT:
                        a_h_opt[idx_r][idx_e][idx_q] = 1
        ...
        if timeout == 0:
            ...
            if t >= 1:
                h_VM_0 = [sum(x_opt[idx_r][idx_v][idx_p]
                            ...
                            for xi       in range(v.Xi_REQ)
                            if  v.Chi_REQ[xi] == (sys.CHI_MAX + 1)
                            ) / p.Xi_MAX[0] + sum((vars_opt["u_{}_{}".format(idx_p,chi)]>LIMIT)
                                               for chi in range(sys.CHI_MAX)
                ...
                cond_1 = sum(c_opt[idx_r][idx_v][idx_p]
                                for idx_r, r in enumerate(reqs)
                                for idx_v, v in enumerate(r.V_S)
                            ) + (sys.C_CON + sys.C_GOS) * h_VM_0[idx_p] + sys.C_HHO * (vars_opt["u_h_{}".format(idx_p)]>LIMIT) + sys.C_GOS * sum(
