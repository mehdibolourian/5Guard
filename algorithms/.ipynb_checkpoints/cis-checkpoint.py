...
def cis_iter(profit_prev, iter, t, r_t, R_t, V_P_S, V_P_R, E_P, E_P_l, L, L_pqi, f_fgr):
    X_t = []
    Y_t = []
    ...
    # Eq. 27
    h_B_h_1 = [[-gp.quicksum(b[idx_r][idx_e][idx_e_p][0][s]
                              for idx_e_p, e_p in enumerate(E_P)
    ...
    h_B_h_2 = [[-gp.quicksum(b[idx_r][idx_e][idx_e_p][0][s]
                              for idx_e_p, e_p in enumerate(E_P)
    ...
    h_B_h_3 = [[-gp.quicksum(b[idx_r][idx_e][idx_e_p][0][s]
                              for idx_e_p, e_p in enumerate(E_P)
    ...
    # Deployment Costs
    g_dep = ( sys.Theta * gp.quicksum(l.B_MAX[0] * a_z1[idx_r][idx_l]
                                      for idx_r, r     in enumerate(reqs)
                                      for idx_l, l     in enumerate(L)
                                     )
...
#                             print(vars_opt["y_{}_{}_{}_{}_{}".format(idx_r,idx_w,idx_q,f,k)])
        ...
        g_dep_B_opt = ( sys.Theta * sum(l.B_MAX[0] * vars_opt["a_z1_{}_{}".format(idx_r,idx_l)]
                                      for idx_r, r in enumerate(reqs)
                                      for idx_l, l in enumerate(L)
                                     )
                    )

        g_dep_C_opt = (sys.Omega * sum(p.C_MAX * vars_opt["a_x1_{}_{}".format(idx_r,idx_p)]
                                      for idx_r, r in enumerate(reqs)
                                      for idx_p, p in enumerate(V_P_S)
                                     )
                    )
        ...
        h_B_h_1 = [[-sum(vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,0,s)]
                         for idx_e_p, e_p in enumerate(E_P)
                         if  (e_p.p in V_P_S) and (e_p.q in V_P_S)
                         for s            in range(S_MAX)
                        ) + B_REQ_EFF[idx_r][idx_e] * h_REV_1[idx_r][idx_e]
        ...
        h_B_h_2 = [[-sum(vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,0,s)]
                         for idx_e_p, e_p in enumerate(E_P)
                         if  (e_p.p in V_P_R) and (e_p.q in V_P_R)
                         for s            in range(S_MAX)
                        ) + B_REQ_EFF[idx_r][idx_e] * h_REV_2[idx_r][idx_e]
        ...
        h_B_h_3 = [[-sum(vars_opt["b_{}_{}_{}_{}_{}".format(idx_r,idx_e,idx_e_p,0,s)]
                         for idx_e_p, e_p in enumerate(E_P)
                         if  (e_p.p in V_P_S) and (e_p.q in V_P_R)
                         for s            in range(S_MAX)
                        ) + B_REQ_EFF[idx_r][idx_e]
